...
def piggy_points(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    ten_digit = 0 if score < 10 else (score%100)//10
    digit = score % 10
    return abs(ten_digit-digit)+4
    # END PROBLEM 2
...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    player_digits = get_digits(player_score)
    opponent_digits = get_digits(opponent_score)
    return min(player_digits) < min(opponent_digits) and max(player_digits) > max(opponent_digits)     
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        player_score += take_turn(num_rolls, opponent_score, dice, goal)
        if who == 0:
            score0 = player_score
        else:
            score1 = player_score
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(score0, score1):
        diff = score0 - last_score if who == 0 else score1 - last_score
...
def make_averaged(original_function, trials_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    def averaged(*arg):
        avg = sum(original_function(*arg) for i in range(trials_count))/trials_count
        return avg
    return averaged
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_score, candidate = -1, -1
    
    averaged_score = make_averaged(roll_dice, trials_count)
    
    for i in range(1, 11):
        score = averaged_score(i, dice)
        if max_score < score:
            max_score, candidate = score, i
